...
//pageVO를 파싱하슨 함수(아래)
	var printPageVO = function(pageVO, target){
		...
		target.html(paging); //.html은 해당되는 target에 출력
	}
...
//댓글 리스트 실행
	$(document).ready(function(){
		...
		//페이징 번호클릭시 페이지이동이 아니고, getPage함수 실행이 되면 OK.
		$(".pagination").on("click","li a", function(event){
			event.preventDefault();//기본 a href 이동 이벤트를 금지
			page = $(this).attr("href"); //href에는 페이지번호(1,2,3..)가 들어가있음.
			getPage("/reply/select/" + bno + "/" + page); //페이지번호를 호출
		});
	});
</script>
			
<script>
//댓글 입력/수정/삭제 버튼 이벤트 처리(Ajax이용해서 url호출=JSON데이터를 get,set)
$(document).ready(function(){
...
$("#insertApplyBtn").on("click",function(){
		...
		//입력값 유효성검사 =validation 처리(아래)
		if(replyer == "" || replytext == ""){
			alert("작성자와 댓글 내용은 필수 입력 값 입니다.")
			return false;
		}
		$.ajax({
			type:'post', 
